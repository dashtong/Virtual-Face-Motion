...
class App():
    ...
    def onmouse(self, event, x, y, flags, param):
        ...
        if event == cv2.EVENT_RBUTTONDOWN:
            ...
            self.tka.writeInTextBox(" Now press the key 'n' a few times until no further change ")
            self.tka.update()
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            if self.rect_over == False:
                ...
                self.tka.writeInTextBox("first draw rectangle ")
                self.tka.update()
    ...
    def run(self, tka):
        self.tka = tka
        self.rawimg = cv2.imread(tka.filename)
        dim = (640, 360)
        ...
        self.tka.writeInTextBox(" Instructions: ")
        self.tka.writeInTextBox(" Draw a rectangle around the object using right mouse button ")
        self.tka.update()
        ...
        while(1):
            ...
            if k == 27:         # esc to exit
                ...
                self.tka.writeInTextBox(" mark background regions with left mouse button ")
                self.tka.update()
                self.value = self.DRAW_BG
            elif k == ord('1'): # FG drawing
                print(" mark foreground regions with left mouse button ")
                self.tka.writeInTextBox(" mark foreground regions with left mouse button ")
                self.tka.update()
                ...
                self.tka.writeInTextBox(" Result saved as image ")
                self.tka.update()
                cv2.destroyAllWindows()
                break
            elif k == ord('r'): # reset everything
                print(" Resetting ")
                self.tka.writeInTextBox(" Resetting ")
                self.tka.update()
                self.rect = (0,0,1,1)
                self.drawing = False
                self.rectangle = False
                ...
                print(" For finer touchups, ressing keys '0' to mark background and '1' to mark the forground and again press 'n'")
                print(" If you are satisfied with the output, press 'enter' to save it")
                self.tka.writeInTextBox(" For finer touchups, ressing keys '0' to mark background and '1' to mark the forground and again press 'n' \n")
                self.tka.writeInTextBox(" If you are satisfied with the output, press 'enter' to save it")
                self.tka.update()
                try:
        ...
        self.tka.writeInTextBox("Done Removing Original Background")
        self.tka.update()

    def changeBg(self, tka):
        bg = cv2.imread(tka.filename)
