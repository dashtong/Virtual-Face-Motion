...
def img2vid(title,hd):
    global fps
    global img_array
    global size

    if hd:
        out = cv2.VideoWriter('final_products/{}_VirtualVideo.mp4'.format(title),cv2.VideoWriter_fourcc(*'mp4v'), fps, tuple(size))
    else:
        out = cv2.VideoWriter('final_products/{}_VirtualVideo.mp4'.format(title),cv2.VideoWriter_fourcc(*'avc1'), fps, tuple(size))
